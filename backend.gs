...
function getPin() {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === 'pin') {
      return ContentService.createTextOutput(JSON.stringify({result: "success", pin: data[i][1].toString()})).setMimeType(ContentService.MimeType.JSON);
    }
  }
  
  sheet.appendRow(['pin', '']);
  return ContentService.createTextOutput(JSON.stringify({result: "success", pin: ""})).setMimeType(ContentService.MimeType.JSON);
}
...
function setPin(newPin) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === 'pin') {
      sheet.getRange(i + 1, 2).setValue(newPin.toString());
      found = true;
      break;
    }
  }
  ...
  if (!found) {
    sheet.appendRow(['pin', newPin.toString()]);
  }
