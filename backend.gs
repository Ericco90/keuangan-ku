...
const MASTER_PIN = "123456"; // GANTI DENGAN PIN RAHASIA ANDA
...
function doPost(e) {
  ...
  try {
    if (action === 'addTransaction') {
      ...
      return deleteBudget(e.parameter.category);
    } else if (action === 'deleteGoal') {
      return deleteGoal(e.parameter.id);
    } else if (action === 'deleteDebt') {
      return deleteDebt(e.parameter.id);
    } else if (action === 'getPin') {
      return getPin();
    } else if (action === 'setPin') {
      return setPin(e.parameter.pin);
    } else {
      return ContentService.createTextOutput(JSON.stringify({result: "error", message: "Invalid action"})).setMimeType(ContentService.MimeType.JSON);
    }
  } catch (error) {
...
function deleteBudget(category) {
  ...
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (data[i][0].toString() === category.toString()) {
      sheet.deleteRow(i + 1);
      return ContentService.createTextOutput(JSON.stringify({result: "success", message: "Berhasil Dihapus"})).setMimeType(ContentService.MimeType.JSON);
    }
  }
...
function getPin() {
  return ContentService.createTextOutput(JSON.stringify({result: "success", pin: MASTER_PIN})).setMimeType(ContentService.MimeType.JSON);
}

function setPin(newPin) {
  return ContentService.createTextOutput(JSON.stringify({result: "success", message: "PIN diubah langsung melalui backend.gs"})).setMimeType(ContentService.MimeType.JSON);
}
